...
void ChainReader::tickSize(TickerId tickerId, TickType field, Decimal size) 
{
  // Determine the strike price and right
  int index = (tickerId - 3) / 2;
  double strike = *std::next(strikes.begin(), index);
  char right = (tickerId & 1) ? 'C' : 'P';
...
void ChainReader::error(int id, int errorCode, const std::string& errorMsg, const std::string& advancedOrderRejectJson) 
{
